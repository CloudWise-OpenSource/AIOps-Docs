...
First we define the forecasting error as:
...
|File name|Relevant repository|
|---|---|
|**MicroSS.zip** |trace | business | run|
...
is the accuracy score of the ith classification;

is the precision score of the ith classificaiton;

is the recall score of the ith classification;

is the f1 score of the ith classification;
...
is total number of samples， is the set of clusters and is the set of classes. represents the cluster in the set , represents the class in the set .
...
Entity uncertainty: Log entities are usually infinite and uncertain in length, such as path entities, which makes it very difficult to identify entities through dictionaries or similarity methods.

Generalization: Log entities from different sources usually have different characteristics. For example, the path in the windows system is different from the path in the Linux system. Beside, NER in logs has higher requirements for identifying entities that have not appeared before, because many entities are completely different in different environments, such as host entities.

High performance requirements: The system and equipment will continue to produce a large number of logs, usually GB-level log files need to be processed. How to optimize the performance of the model so that it can meet NER's computing time requirements is also a major challenge in NER practice.

**>> data description** 

|File name|Relevant repository|
|---|---|
|**Companion Data.zip** |log|
...
|File name|Relevant repository|
|---|---|
|**MicroSS.zip** |metric | trace | business | run|



\iffalse

<< We have trace, log, telemetry, to support the root cause analysis

<< also the business log is provided

<< also business logic is provided.

<< also, the alert on the amonalous metric for root cause analysis, and the real root cause metric will be given, indicating that we know the root causes each time.

\fi

**>> evaluation metrics (formulas later)** 

In MeRCA we use the score , where is 1, 3 or 5, to evaluate the performance of algorithm . We denote as the set of incidents in MeRCA. For each incident and algorithm , the output order for metric is . We also denote as the real root cause for incident . Then we define the evaluation score , where is 1, 3 or 5 .